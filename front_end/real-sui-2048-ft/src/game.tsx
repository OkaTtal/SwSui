...
function App() {
    ...
    for (const obj of allObjs) {
      if (obj.data?.content?.dataType === "moveObject") {
        ...
        if (!fields.gameover) {
          ...
          console.log('发现object_id: ' + obj.data?.objectId); // 这里一定有值    
  ...
  const init = () => {
    ...
    console.log('数据: ' + data)
          ...
          if (grids) {
            console.log('object_id:', objectId); // 检查这里是否有值
            // addNumber(grids);
            // addNumber(grids);
            setGrid(grids);
            addNumber(grids)
...
function resetMilestones() {
  ...
  const doSwipe = (direction: number,l1: number,l2: number,l3: number,l4: number,dummy?: boolean): Grid[] | void => {
    ...
    if(direction==0){
        endnum = 3;
        dif=1
        newGrid = swipe_left_up(endnum,dif,l1,l2,l3,l4,dummy)
    }else if(direction==1){
        endnum=-3;
        dif=1
        newGrid = swipe_right_down(endnum,dif,l1,l2,l3,l4,dummy)
    }else if(direction==2){
        endnum=-12;
        dif=4
        newGrid = swipe_right_down(endnum,dif,l1,l2,l3,l4,dummy)
